...
def strips_layout_two_PT(prs: Presentation, layout_index: int, buyers_chunk_df: pd.DataFrame, start_number: int, brand_api_key):
    ...
    Portuguese version of template_2 (layout B)
    Language: pt
    ...
    copy_table_from_template_slide(prs, source_slide_idx=4, target_slide=slide)
    ...
    for i, (_, row) in enumerate(buyers_chunk_df.iterrows()):
        ...
        for idx, para in enumerate(cell.text_frame.paragraphs):
            if para.runs:
                para.runs[0].text = f"Receita: {revenue}\nEBITDA: {ebitda}\nMarket Cap: {market_cap}\nDívida Total: {total_debt}\nFuncionários: {employees}" # Assumes same formating for the whole text
                found_index = idx
                break
        else:
            # if none of the paragraphs had a run text is added with default formating
